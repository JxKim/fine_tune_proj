...
def train(model,tokenizer,config:SFTConfig):
    ...
    for step in range(num_steps):
        ...
        for ids in batch_data:
            ...
            # padding
            padded_sequence = torch.nn.functional.pad(torch.tensor(ids["input_ids"],dtype=torch.long), (0, padding_length), value=pad_token_id).tolist()
...
def save_model_tokenizer(model,tokenizer,output_dir):
    """
    将模型和tokenizer保存到指定的目录
    """
    print("正在保存模型参数和tokenizer")
...
def main():
    ...
    config = SFTConfig(batch_size=args.batch_size,train_data_size=args.train_data_size,log_dir=args.log_dir)
    train(model,tokenizer,config)
    # 保存模型和tokenizer
    save_model_tokenizer(model,tokenizer,config.output_dir)
